...
const booksHistory = [
  {
    ...
    book: 'As coisas que você só vê quando desacelera',
    author: 'Haemin Sunim',
...
export default function InnerGardenPage() {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      const saved = localStorage.getItem(`notebook-inner-garden-${user.email}-${currentMonth}`);
      if (saved) {
      ...
      booksHistory.forEach(book => {
        const monthData = localStorage.getItem(`notebook-inner-garden-${user.email}-${book.month}`);
        if (monthData) {
  ...
  const saveNotebook = () => {
    setSaving(true);
    localStorage.setItem(`notebook-inner-garden-${user.email}-${currentMonth}`, JSON.stringify(notebook));
            ...
            <div className="p-8" style={{ backgroundImage: 'repeating-linear-gradient(transparent, transparent 31px, rgba(184,217,192,0.3) 31px, rgba(184,217,192,0.3) 32px)' }}>
              ...
              <div className="mb-8">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <label className="font-serif text-lg" style={{ color: '#4A5D4F' }}>Citações Favoritas</label>
                </div>
                {isEditing ? (
                  <textarea
                    ...
                    style={{ backgroundColor: 'rgba(215,237,221,0.1)', borderColor: '#B8D9C0', color: '#171717', fontStyle: 'italic' }}
                  />
                ) : (
              ...
              <div className="mb-8">
                <div className="flex items-center gap-2 mb-3">
                  <span style={{ color: '#B8D9C0' }}>♡</span>
                  <label className="font-serif text-lg" style={{ color: '#4A5D4F' }}>Minhas Reflexões</label>
                </div>
                {isEditing ? (
                  <textarea
                    ...
                    className="w-full h-32 p-4 rounded-xl border-2 focus:outline-none resize-none transition-all"
                    style={{ backgroundColor: 'rgba(215,237,221,0.1)', borderColor: '#B8D9C0', color: '#171717' }}
                  />
                ) : (
                  <div className="p-4 rounded-xl min-h-[80px]" style={{ backgroundColor: 'rgba(215,237,221,0.1)' }}>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <span style={{ color: '#B8D9C0' }}>?</span>
                  <label className="font-serif text-lg" style={{ color: '#4A5D4F' }}>Perguntas para o Encontro</label>
                </div>
                {isEditing ? (
                  <textarea
                ...
                <div key={book.month}>
                  ...
                  >
                    <div 
                      ...
                      style={{ 
                        backgroundColor: isOpen ? 'rgba(184,217,192,0.3)' : '#FFFEF9',
                        boxShadow: '0 2px 8px rgba(0,0,0,0.04), 0 0 0 1px rgba(184,217,192,0.2)',
                      }}
                    >
                      {/* Ícone da pasta */}
                      <div 
                        className="w-14 h-12 rounded-lg flex items-center justify-center relative"
                        style={{ backgroundColor: '#B8D9C0' }}
                      >
                        <div 
                      ...
                      <div className="flex-1">
                        <h3 className="font-serif text-lg" style={{ color: '#4A5D4F' }}>{book.monthLabel}</h3>
                        <p className="text-sm" style={{ color: 'rgba(74,93,79,0.6)' }}>{book.book}</p>
                      </div>
                      
                      {/* Indicador */}
                      <div className="flex items-center gap-2">
                        {hasNotes && (
                          <span className="text-xs px-2 py-1 rounded-full" style={{ backgroundColor: 'rgba(184,217,192,0.3)', color: '#4A5D4F' }}>
                            Com anotações
                          </span>
                        )}
                        <span style={{ color: '#B8D9C0', transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}>
                          ▼
                        </span>
                      </div>
                    </div>
                    ...
                    >
                      {hasNotes ? (
                        <>
                          {monthData.citacoes && (
                            <div className="mb-6">
                              <p className="text-sm font-medium mb-2" style={{ color: '#4A5D4F' }}>❝ Citações Favoritas</p>
                              <p className="italic text-sm" style={{ color: 'rgba(74,93,79,0.8)', whiteSpace: 'pre-wrap' }}>{monthData.citacoes}</p>
                            </div>
                          )}
                          {monthData.reflexoes && (
                            <div className="mb-6">
                              <p className="text-sm font-medium mb-2" style={{ color: '#4A5D4F' }}>♡ Minhas Reflexões</p>
                              <p className="text-sm" style={{ color: 'rgba(74,93,79,0.8)', whiteSpace: 'pre-wrap' }}>{monthData.reflexoes}</p>
                            </div>
                          )}
                          {monthData.perguntas && (
                            <div>
                              <p className="text-sm font-medium mb-2" style={{ color: '#4A5D4F' }}>? Perguntas</p>
                              <p className="text-sm" style={{ color: 'rgba(74,93,79,0.8)', whiteSpace: 'pre-wrap' }}>{monthData.perguntas}</p>
                            </div>
                          )}
                        </>
                      ) : (
                        <p className="text-sm text-center py-4" style={{ color: 'rgba(74,93,79,0.4)' }}>
